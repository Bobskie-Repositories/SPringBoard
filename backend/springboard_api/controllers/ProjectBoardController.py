...
class CreateProjectBoard(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        prompt = (f'''Considering the rules: {...}, please conduct a critical analysis of the following data regarding the startup idea: {...}. The content should fit in the rules.Assign a numerical rating (1-10), not in string, for each of the following aspects, evaluating its potential and feasibility:
...
Ratings below 5 should indicate deficiencies in composition, effort, verbosity, or information. Give low scores to contents that does not follow the rules. Give very low scores to severely not following the rules or instructions. This is the template {...}. Those sentences enclosed in [] are just instructions. Give 0 scores to contents that did't answer the template. Maintain a critical and practical approach when assigning ratings. For each aspect, provide at least two specific recommendations for improvement and two sentences of feedback on the presentation and structure of the data, along with suggestions for enhancement. The output should be in the following JSON format:
'desirability': 'numerical rating', 'feasibility': 'numerical rating', 'viability': 'numerical rating', 'recommendations_desirability': ['specific advice'], 'recommendations_feasibility': ['specific advice'], 'recommendations_viability': ['specific advice'], 'feedback_desirability': ['specific feedback'], 'feedback_feasibility': ['specific feedback'], 'feedback_viability': ['specific feedback'].
Ensure a fair and balanced assessment for each aspect, maintaining a conservative mindset to reserve higher ratings for truly exceptional ideas. Approach the evaluation with skepticism, requiring clear evidence of groundbreaking innovation, robust feasibility, and undeniable market demand to justify ratings above 6. Ratings of 8, 9 or 10 should be sparingly awarded, reserved only for ideas demonstrating significant potential to exceed market standards and effectively address real-world problems.This evaluation method is designed to ensure that only ideas with undeniable merit and a clear pathway to successful implementation receive high scores. Your conservative ratings and detailed justifications will help identify areas where the idea needs significant refinement or rethinking.'''
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            prompt = (
                ...
                f"\n2. Feasibility: Consider the technical and operational feasibility of implementing the proposed idea or solution. Evaluate the availability of resources, expertise, technology, and infrastructure needed for successful development and delivery. "
                f"\n3. Viability: Assess the economic and financial sustainability of the proposed venture. Determine its ability to generate revenue and achieve profitability over the long term. "
                f"\nRatings below 5 should indicate deficiencies in composition, effort, verbosity, or information. Give low scores to contents that does not follow the rules. Give very low scores to severely not following the rules or instructions. This is the template {template.content}. Those senteces enclosed in [] are just instructions. Give 0 scores to contents that didn't answer the template. "
